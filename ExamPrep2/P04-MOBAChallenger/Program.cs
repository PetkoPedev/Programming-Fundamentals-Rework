    ...
    class Program
    {
        static void Main(string[] args)
        {
                ...
                else if (inputType.Count == 2)
                {
                    ...
                    if (players.ContainsKey(firstPlayer) && players.ContainsKey(secondPlayer))
                    {
                        ...
                        foreach (var pos2 in players[secondPlayer].Keys)
                        {
                            ...
                            {
                                int totalSkillFP = players[firstPlayer].Values.Sum();
                                int totalSkillSP = players[secondPlayer].Values.Sum();
                                if (totalSkillFP > totalSkillSP)
                                {
                                    players.Remove(secondPlayer);
                                    break;
                                }
                                else if (totalSkillFP < totalSkillSP)
                                {
            ...
            foreach (var p in players.OrderByDescending(x => x.Value.Values.Sum()).ThenBy(x => x.Key))
            {
                ...
                foreach (var pos in p.Value.OrderByDescending(s => s.Value).ThenBy(b => b.Key))
                {
